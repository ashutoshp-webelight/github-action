...
from core.utils.middleware import ResponseMiddleware
...
def start_exception_handlers(_app: FastAPI) -> None:
    ...
    async def exception_handler(*args) -> JSONResponse:
        """
        Handler for all the :`Exception` raised within the app.
        """
        exc = args[1]
    ...
    async def custom_exception_handler(*args) -> JSONResponse:
        ...
        return JSONResponse(
            status_code=exc.status_code, content={"status": "error", "code": exc.status_code, "message": exc.message}
        )
...
def create_app(debug: bool = False) -> FastAPI:
    ...
    _app = FastAPI(
        ...
        openapi_url="/openapi.json" if settings.ENV != AppEnvironment.Production else None,
        docs_url="/docs" if settings.ENV != AppEnvironment.Production else None,
        redoc_url="/redoc" if settings.ENV != AppEnvironment.Production else None,
    )
